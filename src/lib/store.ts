...
export async function saveAudit(report: AuditReport): Promise<void> {
  ...
  try {
    ...
    await admin.from("audits").upsert({
      ...
      paid_search_score: report.scores.paidSearch,
      landing_page_score: report.scores.landingPage,
      tracking_score: report.scores.tracking,
      keyword_score: report.scores.keyword,
      budget_waste_score: report.scores.budgetWaste,
